...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        code: [{ validator: this.checkcode, trigger: "change" }],
      },
  ...
  methods: {
    ...
    locationInfo() {
      ...
      if (!code) {
        ...
        if (this.locationArrShow) {
          this.locationArrShow.forEach((item) => {
            if (item["code"] === code) {
              this.branchID = item["code"];
              this.pickupLocation =
                this.language === "zh-CN" ? item["cn"] : item["en"];
            }
          });
        }
      }
    ...
    confirmbtn(locationform) {
      this.$refs[locationform].validate((valid) => {
        if (valid) {
          let item = this.locationform.code;
        ...
        if (valid) {
          ...
          if (formdatareceClone.cardholderid === "") {
            ...
            let info = {
              ...
              details: JSON.stringify(formdatareceClone.details),
        ...
        if (valid) {
          ...
          if (formMailClone.cardholderid === "") {
            ...
            let info = {
              ...
              details: JSON.stringify(formMailClone.details),
  ...
  computed: {
    ...mapState(["tableCellStyle", "locationArrShow"]),
    ...mapGetters([
