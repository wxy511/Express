  ...
  methods: {
    ...
    handleLogin() {
      ...
      this.$refs.loginForm.validate((valid) => {
        if (valid) {
          that.loading = true;
          that.loginForm.secret = that.$md5(that.loginForm.pwdaes);
          that.$store
            ...
            .then((res) => {
              ...
              else if(res.error === 21 || res.error === 22){
                that.loginErr = i18n.t("Error.ErrorPassword");
              }
              else if(res.error === 101){
                that.loginErr = i18n.t("Error.VaildCodeError");
              }  
              else if(res.error === 142){
                that.loginErr = i18n.t("Error.PwdExpired");
              }               
              else{
                that.loginErr = i18n.t("erGeneral");
              }
            })
            .catch((error) => {
              that.loading = false;
